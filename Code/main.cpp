...
std::pair<MMatrix, int> SDLS(const MMatrix &A, const MVector &b, MVector &x0,
                             int MaxIt = 1000, double Tol = 1e-6) {
  int n = x0.size(), iter;
  MMatrix X(
      n, MaxIt + 1,
      0.0); // Use MMatrix X to store all vector solution from the algorithm
  MVector r = A.transpose() * (b - A * x0);
  MVector x = x0;
  X.vector_evalue(0, x0);
  for (iter = 1; iter < MaxIt; iter++) {
    double alpha =
        (Transpose(r) * r)[0] / (Transpose(r) * A.transpose() * A * r)[0];
    x += alpha * r;
    ...
    if (r.LTwoNorm() < Tol)
      break; // Check if solution is accurate enough
  }
...
// Note that NIHT doesn't always converge, the viable iter is used to judge
// whether the algorithm converges or not. If R.size() > 1, that is to say, iter
// > 1, the algorithm converges. Whereas if R.size() = 1, the algorithm does not
// converge.
std::pair<MVector, MVector> NIHT(const MMatrix &A, const MVector &b, int k,
                                 int MaxIt = 1e3, double Tol = 1e-6) {
  int iter;
  // Initialize x, r
  ...
  for (iter = 0; iter < MaxIt; iter++) {
    ...
    r = A.transpose() * (b - A * x);
    R[iter + 1] = r.LTwoNorm();
    if (R[iter + 1] > R[iter]) { // If the residuals donot descend, the
  ...
  if (A.Rows() < 2 * k && (A * x - b).LTwoNorm() > Tol)
    iter = -1;
...
void exercise_main() {
  ...
  A_3 = {1, 2, 2, 1, 3, -3};
  b = {10, -1, 0};
  std::pair<MMatrix, int> sol_1 = SDLS(A_1, b, x0), sol_2 = SDLS(A_2, b, x0),
                          sol_3 = SDLS(A_3, b, x0);
  ...
  data1_3.close();
  // Print the outcome
  std::cout << "The solution of the problem is x = "
            << sol_1.first.last_column() << std::endl
            << "Iteration step = " << sol_1.second << std::endl;
  std::cout << "last row: [-1 0], iteration step = " << sol_1.second
            << std::endl
            << "last row: [1.8 -2], iteration step = " << sol_2.second
            << std::endl
            << "last row: [-2 -2], iteration step = " << sol_3.second
            << std::endl;
}
} // namespace Exercise1
