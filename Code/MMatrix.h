...
public:
  ...
  double operator()(int i, int j) const { return A[j + i * nCols]; }

  // access element, indexed by (row, column) [lvalue]
  double &operator()(int i, int j) { return A[j + i * nCols]; }
...
inline MMatrix operator*(const MMatrix &lm, const MMatrix &rm) {
  ...
  if (lm.Cols() ==
      rm.Rows()) { // Continue the calculation if the number lm's col = rm's row
...
// When size of MMatrix A is 1*1, we can treat it as a double variable in
// division
